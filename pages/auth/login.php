...
if(isset($_POST['login'])){
  $email = $_POST['email'];
  $pass = $_POST['pass'];
  $checkEmail = "Select * from child where email='$email'";
  $emailResult = mysqli_query($con, $checkEmail);
  if(mysqli_num_rows($emailResult) == 0){
    $err = 'Email does not exist!';
  }else{
    $checkPass = "select * from child where email='$email' and password='$pass'";
    $passResult = mysqli_query($con, $checkPass);
    if(mysqli_num_rows($passResult) == 0){
      $err = 'Incorect Password!';
    } else {
      $query = "SELECT * FROM child WHERE email='$email' AND password='$pass'";
